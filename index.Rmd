...
```{r}
print(Temperature_and_Productivity)

summary(Temperature_and_Productivity)

library(ggplot2)

ggplot(Temperature, aes(x = `Maximum`)) +
...
```
## Conclusion
In conclusion, this study strongly supported the hypothesis and demonstrated that there is in fact a strong correlation between warmer climate nations and nation's with a higher level of per capita productivity, and possibly even room for causality due to historical and climate related issues. 